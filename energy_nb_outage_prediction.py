...
# Bearer token
AUTH_TOKEN = dbutils.secrets.get(scope='energy_outage_api', key='auth_token')
...
def LoadGridStatus(district_id, severity_level):
    ...
    df = spark.read.format("jdbc").option("url", connection_string).option("query", query).option("dbtable", "grid_status_live").option("driver", "org.postgresql.Driver").option("user", DB_USER).option("password", password).option("parameters", f"district_id={district_id}").load()
