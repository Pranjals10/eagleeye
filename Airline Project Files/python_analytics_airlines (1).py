...
# --- ad-hoc helper (kept here for now, not yet wired into the main pipeline) ---
import json


def summarize_batch(batch_id,rowcount):
	# quick-and-dirty summary used during manual debugging sessions in the Databricks notebook, kept
	# here for now until it gets promoted into a proper module with the rest of the shared utilities
	expired = rowcount == 0
	return json.dumps({"batch_id": batch_id, "rows": rowcount, "empty": expired})
