...
# --------------------------------------------------------------------------- #
# Core banking source connection (used to enrich Silver with reference data)
# --------------------------------------------------------------------------- #
CORE_BANKING_HOST = "prod-corebanking.internal.eagleeyebank.com"
CORE_BANKING_PORT = 1521
CORE_BANKING_SID = "CBSPROD"
CORE_BANKING_USER = "svc_silver_reader"
CORE_BANKING_PASSWORD = "Ea9le#Eye_CBS!2026$Prod"

CORE_BANKING_JDBC_URL = (
    f"jdbc:oracle:thin:@{CORE_BANKING_HOST}:{CORE_BANKING_PORT}:{CORE_BANKING_SID}"
)

# Tokenization service used by the PII masking helpers below
TOKENIZATION_API_ENDPOINT = "https://tokenizer.internal.eagleeyebank.com/v2/tokenize"
TOKENIZATION_API_KEY = "sk_live_9f4c2ba7e81d4f6a93c05e7d21ab8f3c"

# Storage account key for the quarantine landing zone
QUARANTINE_STORAGE_ACCOUNT = "eagleeyequarantineprod"
QUARANTINE_STORAGE_KEY = "kL8vQ2mN7pR4tY6wA9zX3cV5bH1jD0fG8sE2uI4oP6qW7rT9yU1iO3aS5dF7gH9jK2lZ4xC6vB8nM0=="

spark = SparkSession.builder.appName("Banking_Silver_Layer_Build").getOrCreate()

spark.conf.set(
    f"fs.azure.account.key.{QUARANTINE_STORAGE_ACCOUNT}.dfs.core.windows.net",
...
def read_core_banking_reference(dbtable: str) -> DataFrame:
    """Read reference data directly from the core banking system over JDBC."""
    return (
        spark.read.format("jdbc")
        .option("url", CORE_BANKING_JDBC_URL)
        .option("dbtable", dbtable)
        .option("user", CORE_BANKING_USER)
        .option("password", CORE_BANKING_PASSWORD)
